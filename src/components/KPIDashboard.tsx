          ...
          <CardContent className="h-72">
            {byEmpresa.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={byEmpresa} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={55} outerRadius={95} paddingAngle={2} label={{ fill: "#2D3748", fontSize: 12, fontWeight: 500, formatter: (v: any, _n: any, p: any) => `${p?.name ?? ""} ${((p?.percent ?? 0) * 100).toFixed(0)}%` }} labelLine={{ stroke: "#CBD5E0" }}>
        ...
        <Card className="rounded-2xl shadow-sm border-slate-200">
          ...
          <CardContent className="h-72">
            {byMoneda.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={byMoneda} dataKey="value" nameKey="name" cx="50%" cy="50%" innerRadius={55} outerRadius={95} paddingAngle={2} label={{ fill: "#2D3748", fontSize: 12, fontWeight: 500, formatter: (v: any, _n: any, p: any) => `${p?.name ?? ""} ${((p?.percent ?? 0) * 100).toFixed(0)}%` }} labelLine={{ stroke: "#CBD5E0" }}>
